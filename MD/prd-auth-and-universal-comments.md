...
- [ ] **2.3 API 路由服务端鉴权**
  - 在以下 API 路由中校验 `getUserRole()` === `'admin'`，否则返回 403：
    ...
    - [ ] `POST /api/items`（上传）
    - [ ] `PATCH /api/items/[id]`（编辑金额、备注、决策、分类）
...
- [ ] **5.2 RLS（Row Level Security）规则更新**
