import { Suspense } from 'react'
import ImageGrid from '@/components/ImageGrid'
import DraggableImageGrid from '@/components/DraggableImageGridWrapper'
import UploadZone from '@/components/UploadZone'
import SortControl from '@/components/SortControl'
import FinalListToggle from '@/components/FinalListToggle'
import TournamentEntry from '@/components/TournamentEntry'
import { supabaseAdmin } from '@/lib/supabase-admin'
import type { TemplateConfig } from '@/lib/templates'

interface SessionItemsSectionProps {
  sessionId: string
  sessionToken: string
  sort: string
  view: string
  budget: number | null
  isAdmin: boolean
  templateConfig?: TemplateConfig
}

interface Item {
  id: string
  image_url: string
  decision: 'buy' | 'skip' | 'pending'
  price: number | null
  position: number
  category: string | null
  created_at: string
  avgScore: number | null
  arthurScore: number | null
  graceScore: number | null
  commentCount: number
  rank: number | null
}

export function SessionItemsSectionFallback({ isAdmin }: { isAdmin: boolean }) {
  return (
    <div className="space-y-6">
      <div className="h-20 rounded-2xl bg-card animate-pulse" />
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="h-12 rounded-2xl bg-muted animate-pulse sm:w-56" />
        {isAdmin ? <div className="h-10 rounded-2xl bg-muted animate-pulse sm:w-40" /> : null}
      </div>
      {isAdmin ? <div className="h-32 rounded-2xl bg-card animate-pulse" /> : null}
      <div className="h-10 rounded-xl bg-muted animate-pulse w-48" />
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
        {Array.from({ length: 8 }).map((_, index) => (
          <div key={index} className="aspect-square rounded-xl bg-card animate-pulse" />
        ))}
      </div>
    </div>
  )
}

function sortItems(items: Item[], sort: string) {
  return [...items].sort((a, b) => {
    switch (sort) {
      case 'rating':
        if (a.avgScore === null && b.avgScore === null) return 0
        if (a.avgScore === null) return 1
        if (b.avgScore === null) return -1
        return b.avgScore - a.avgScore
      case 'arthur':
        if (a.arthurScore === null && b.arthurScore === null) return 0
        if (a.arthurScore === null) return 1
        if (b.arthurScore === null) return -1
        return b.arthurScore - a.arthurScore
      case 'grace':
        if (a.graceScore === null && b.graceScore === null) return 0
        if (a.graceScore === null) return 1
        if (b.graceScore === null) return -1
        return b.graceScore - a.graceScore
      case 'price':
        if (a.price === null && b.price === null) return 0
        if (a.price === null) return 1
        if (b.price === null) return -1
        return a.price - b.price
      case 'position':
        return a.position - b.position
      default:
        return 0
    }
  })
}

export default async function SessionItemsSection({
  sessionId,
  sessionToken,
  sort,
  view,
  budget,
  isAdmin,
  templateConfig,
}: SessionItemsSectionProps) {
  const { data: rawItems } = await supabaseAdmin
    .from('items')
    .select('id, image_url, decision, price, position, category, created_at, rank, ratings(score, author, scores), comments(id)')
    .eq('session_id', sessionId)
    .order('created_at', { ascending: true })

  const items: Item[] = (rawItems ?? []).map((item) => {
    const ratings = (item.ratings as { score: number; author: string }[]) ?? []
    const scores = ratings.map((rating) => rating.score)
    const avgScore = scores.length > 0
      ? scores.reduce((left, right) => left + right, 0) / scores.length
      : null
    const arthurScore = ratings.find((rating) => rating.author === 'Arthur')?.score ?? null
    const graceScore = ratings.find((rating) => rating.author === 'Grace')?.score ?? null

    return {
      id: item.id,
      image_url: item.image_url,
      decision: item.decision,
      price: item.price,
      position: item.position,
      category: item.category,
      created_at: item.created_at,
      rank: item.rank,
      avgScore,
      arthurScore,
      graceScore,
      commentCount: (item.comments as unknown[])?.length ?? 0,
    }
  })

  const isFinalView = view === 'final'
  const displayItems = isFinalView ? items.filter((item) => item.decision === 'buy') : items
  const sortedItems = sortItems(displayItems, sort)
  const buyCount = items.filter((item) => item.decision === 'buy').length
  const pendingCount = items.filter((item) => item.decision === 'pending').length
  const tournamentItems = items.filter((item) => item.decision === 'buy')
  const totalBuyPrice = items
    .filter((item) => item.decision === 'buy' && item.price !== null)
    .reduce((sum, item) => sum + (item.price ?? 0), 0)
  const itemLabel = templateConfig?.itemLabel || '位'

  return (
    <>
      {items.length > 0 && (
        <div className="flex items-center gap-3 bg-card border border-border rounded-2xl px-5 py-4 shadow-sm mb-6 text-sm">
          <div className="flex-1 flex items-center gap-4 flex-wrap">
            <span className="text-muted-foreground">共 <span className="font-semibold text-foreground">{items.length}</span> {itemLabel}</span>
            <span className="text-green-600 dark:text-green-400 font-medium">已选 <span className="font-bold">{buyCount}</span> {itemLabel}</span>
            <span className="text-muted-foreground font-medium">待处理 <span className="font-semibold">{pendingCount}</span> {itemLabel}</span>
            {totalBuyPrice > 0 && <span className="text-primary font-bold">合计 ¥{totalBuyPrice}</span>}
          </div>
          {budget && (
            <span className={`text-xs shrink-0 font-bold px-2 py-1 rounded-lg bg-muted/50 ${
              totalBuyPrice > budget ? 'text-destructive bg-destructive/10' : 'text-muted-foreground'
            }`}>
              预算 ¥{budget}
              {totalBuyPrice > budget && ' ⚠️ 超额'}
            </span>
          )}
        </div>
      )}

      {items.length > 0 && (
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <Suspense fallback={<div className="h-12 animate-pulse bg-muted rounded-2xl" />}>
            <FinalListToggle current={view} />
          </Suspense>
          {isAdmin ? (
            <TournamentEntry items={tournamentItems} sessionToken={sessionToken} templateConfig={templateConfig} />
          ) : null}
        </div>
      )}

      {isAdmin && !isFinalView ? (
        <div className="mb-8 p-1 bg-card/50 border border-dashed border-border rounded-2xl">
          <UploadZone sessionToken={sessionToken} templateConfig={templateConfig} />
        </div>
      ) : null}

      {sortedItems.length > 1 && (
        <div className="mb-6">
          <Suspense fallback={<div className="h-10 animate-pulse bg-muted rounded-xl" />}>
            <SortControl current={sort} />
          </Suspense>
        </div>
      )}

      {isFinalView && (
        <div className="mb-6 flex flex-col gap-4 rounded-2xl border border-primary/10 bg-primary/5 p-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0 flex-1">
            <h2 className="text-lg font-bold text-foreground leading-tight">✨ 最终清单</h2>
            <span className="mt-1 block text-xs leading-5 text-muted-foreground font-medium">挑选出的 {sortedItems.length} {itemLabel} {templateConfig?.name === '衣评' ? '穿搭' : '优选'}</span>
          </div>
          {totalBuyPrice > 0 && (
            <div className="rounded-xl bg-background/70 px-3 py-2 text-left shadow-sm sm:text-right">
              <span className="block text-[10px] text-muted-foreground uppercase font-bold">预估支出</span>
              <span className="text-xl font-black text-primary">¥{totalBuyPrice}</span>
            </div>
          )}
        </div>
      )}

      {sort === 'position' ? (
        <DraggableImageGrid items={sortedItems} sessionToken={sessionToken} />
      ) : (
        <ImageGrid items={sortedItems} sessionToken={sessionToken} templateConfig={templateConfig} />
      )}
    </>
  )
}