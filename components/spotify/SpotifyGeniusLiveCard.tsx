...
function GeniusCardInner() {
  ...
  const displayAnnotations = (geniusData && geniusData.annotations)
    ? (currentId 
        ? geniusData.annotations.filter(a => a.id === currentId)
        : geniusData.annotations.slice(0, 15))
    : []

  return (
    <div 
      className="rounded-[32px] border border-border/60 bg-card/95 p-6 sm:p-8 shadow-[0_22px_70px_rgba(0,0,0,0.05)] group/genius transition-all duration-500 overflow-hidden h-[720px] flex flex-col"
    ...
    >
      ...
      <div className="flex items-center justify-between mb-8 flex-shrink-0">
        <div>
          ...
          <h3 className="mt-2 text-3xl font-bold tracking-tight text-foreground flex items-center gap-3">
            Behind the Lyrics
            <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-yellow-400/10 text-yellow-500/80 tracking-widest uppercase">Premium</span>
          </h3>
          {geniusData && (
        ...
        <div className="flex flex-col items-end gap-1">
          ...
          <div className="flex items-center gap-1.5">
             {[1, 2, 3].map(i => (
               <div key={i} className={`w-0.5 rounded-full bg-yellow-400/40 transition-all duration-500 ${currentId ? 'h-4' : 'h-2'}`} style={{ transitionDelay: `${i*100}ms` }} />
             ))}
          </div>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start flex-grow overflow-hidden h-full pb-8">
          {/* Left: Lyrics Content (Line-by-line) */}
          <div className="lg:col-span-2 relative h-full flex flex-col overflow-hidden">
            <div className="flex-grow overflow-y-auto scrollbar-hide pr-4 flex flex-col gap-0 isolate">
              {hasLyrics ? (
                geniusData!.lyrics!.split('\n')
                  .filter(line => line.trim() !== '') // 核心修复：过滤空行
                  .map((line, idx) => {
                  ...
                  return (
                    <div 
                      ...
                      className={`px-3 py-2.5 transition-all duration-300 group/line relative select-none [transform:translateZ(0)] will-change-transform ${
                        isLabel 
                          ...
                          : 'text-[22px] md:text-[25px] tracking-tight leading-[1.3]'
                      } ${
                        associatedAnn ? 'cursor-pointer' : ''
                      }`}
                      onMouseEnter={() => associatedAnn && setActiveAnnId(associatedAnn.id)}
                    ...
                    >
                      ...
                      {associatedAnn && (
                        <div className={`absolute left-0 top-1/2 -translate-y-1/2 -translate-x-full pr-3 transition-all duration-300 ${
                          (isActive || isFixed) ? 'opacity-100 scale-100' : 'opacity-0 scale-50 group-hover/line:opacity-100 group-hover/line:scale-100'
                        }`}>
                           <div className={`w-1.5 h-1.5 rounded-full ${isFixed ? 'bg-yellow-400 scale-125' : 'bg-yellow-400/40'} shadow-[0_0_10px_rgba(250,204,21,0.8)]`} />
                        </div>
                      )}
                    </div>
                  )
          ...
          <div 
            className="space-y-4 transition-all duration-500 ease-out h-full flex flex-col overflow-hidden"
          >
            <h4 className="text-[11px] font-bold uppercase tracking-widest text-muted-foreground/60 mb-2 px-1 flex-shrink-0 flex items-center justify-between">
              ...
              {currentId && (
                <button 
                  ...
                  className="text-yellow-500 text-[9px] hover:underline"
                >
                  Clear Selection
                </button>
              )}
            </h4>
            <div className="flex-grow overflow-y-auto scrollbar-hide space-y-4 pr-1 pb-10">
              {displayAnnotations.map((ann) => {
                const isActive = currentId === ann.id
                return (
                  <div
                    key={ann.id}
                    className={`p-5 rounded-[26px] border transition-all duration-500 group/ann cursor-default shadow-sm animate-in fade-in slide-in-from-right-4 ${
                      isActive 
                        ? 'bg-yellow-400/[0.08] border-yellow-400/40 shadow-yellow-400/5' 
                        : 'bg-muted/30 border-border/40 hover:bg-muted/50'
                    }`}
                  >
                    {/* Fragment Display */}
                    {ann.fragment && (
                      <div className="mb-4 pb-3 border-b border-yellow-400/10">
                        <p className="text-[10px] font-bold text-yellow-600/60 dark:text-yellow-400/40 uppercase tracking-widest mb-1.5">Context</p>
                    ...
                    <div className={`mt-4 flex items-center justify-between transition-all duration-500 ${
                      ...
                      <div className="flex items-center gap-2">
                         <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full uppercase tracking-tighter ${
                           isActive ? 'bg-yellow-400 text-black' : 'bg-yellow-400/10 text-yellow-600'
                         }`}>
                           Annotation
                         </span>
                      ...
                      <a href={ann.url} target="_blank" className="p-1.5 rounded-full hover:bg-yellow-400/20 transition-colors">
