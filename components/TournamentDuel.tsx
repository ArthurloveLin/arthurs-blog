        ...
        {results ? (
          ...
          <div className="w-full max-w-5xl">
            <div className="text-center mb-16 space-y-2">
              <h2 className="text-4xl font-black text-white tracking-tighter italic uppercase animate-in fade-in slide-in-from-bottom-4 duration-700">
                Pick Your <span className="text-transparent bg-clip-text bg-gradient-to-r from-pink-500 to-violet-500">Favorite</span>
              </h2>
              <p className="text-white/30 text-sm font-medium tracking-widest uppercase">Select the one that sparks more joy</p>
            </div>

            <div className="grid grid-cols-2 gap-8 md:gap-20 relative h-[450px] md:h-[550px] items-center">
              {/* VS Divider */}
              <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-30 pointer-events-none">
                <div className={`w-16 h-16 rounded-full bg-white text-black font-black flex items-center justify-center shadow-[0_0_50px_rgba(255,255,255,0.3)] border-4 border-black transition-transform duration-300 ${animating ? 'scale-150 animate-vs-impact' : 'scale-100'}`}>
                  VS
                </div>
              </div>
    ...
    >
      <div className={`relative h-full aspect-[3/4.5] rounded-[2rem] overflow-hidden border-2 transition-all duration-500 shadow-2xl ${
        ...
        <div className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-all duration-500 flex flex-col items-center justify-center">
            <div className={`w-20 h-20 rounded-full bg-white/10 backdrop-blur-xl flex items-center justify-center text-white text-4xl shadow-2xl transition-transform duration-500 group-hover:scale-110 group-active:scale-90 border border-white/20 ${isWinner ? 'animate-winner-pulse bg-violet-500/40' : ''}`}>
              {side === 'left' ? '👈' : '👉'}
            </div>
            <div className="mt-6 px-6 py-2 rounded-full bg-white text-black font-black text-xs tracking-widest uppercase shadow-2xl">
              Select
            </div>
        </div>
      ...
      {item.price !== null && (
        <div className="mt-6 text-center">
          <span className="px-4 py-1.5 rounded-full bg-white/5 border border-white/10 text-white font-mono text-lg font-black group-hover:bg-white/10 transition-colors">
            ¥{item.price}
          </span>
        </div>
      )}
  ...
  return (
    <div className="text-center w-full max-w-4xl animate-in fade-in zoom-in duration-1000">
      <div className="mb-12 relative">
        ...
        <span className="inline-block px-5 py-2 rounded-full bg-gradient-to-r from-yellow-500/20 to-orange-500/20 text-yellow-500 text-xs font-black tracking-[0.2em] uppercase mb-6 border border-yellow-500/20 shadow-[0_0_30px_rgba(234,179,8,0.1)]">
          🏆 Tournament Finished
        </span>
        <h2 className="text-6xl font-black text-white mb-4 tracking-tighter">Your Wardrobe <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 via-orange-500 to-pink-500">Elite</span></h2>
        <p className="text-white/40 text-sm font-medium">The results are in. These pieces represent your ultimate style.</p>
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 items-end px-4">
        {/* Runner Up */}
        <div className="order-2 md:order-1 animate-in slide-in-from-bottom-8 duration-700 delay-200">
          <AwardCard item={results.runnerUp} rank={2} label="Runner Up" color="silver" />
        </div>
        
        {/* Champion */}
        <div className="order-1 md:order-2 scale-110 z-10 animate-in slide-in-from-bottom-12 duration-1000">
          <AwardCard item={results.champion} rank={1} label="Champion" color="gold" />
        </div>
        
        {/* Third Place */}
        {results.thirdPlace && (
          <div className="order-3 animate-in slide-in-from-bottom-8 duration-700 delay-400">
